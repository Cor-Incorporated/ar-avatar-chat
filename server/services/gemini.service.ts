...
async function renderResponse(apiKey: string, prompt: string, attachments: ChatAttachment[], history: ConversationHistoryItem[], context: RequestContext, calendar?: CalendarResult): Promise<GeminiResponse> {
  process.env.GOOGLE_GENERATIVE_AI_API_KEY = apiKey;
  const result = await generateText({
    model: google(getGeminiModel()), system: `${SYSTEM}\n${buildCurrentTimeInstruction(context)}`,
...
export async function handleFunctionCalling(
  ...
  provider?: CalendarProvider, context: RequestContext = createRequestContext()
): Promise<GeminiResponse> {
  const temporalFact = resolveTemporalFact(userPrompt, context);
  if (temporalFact) return temporalFactResponse(temporalFact);
  if (!isCalendarIntent(userPrompt)) return renderResponse(apiKey, userPrompt, attachments, conversationHistory, context);
  ...
  try {
    ...
    const response = await renderResponse(apiKey, userPrompt, attachments, conversationHistory, context, calendarResult);
